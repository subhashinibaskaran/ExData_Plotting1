...
voltage <- as.numeric(subsetofhhpwr$Voltage)
globalreactivepower <- as.numeric(subsetofhhpwr$Global_reactive_power)
datetime <- strptime(paste(subsetofhhpwr$Date, subsetofhhpwr$Time, 
                           sep=" "), "%d/%m/%Y %H:%M:%S")

## Open a png file plot4.R with height and width set to 480 pixels
...
## plot the graph for voltage
plot(datetime, voltage, type = "l", xlab = "datetime", ylab = "Voltage")

## plot the graph for global reactive power
